...
class GenerationSafetyTests(unittest.TestCase):
    ...
    SESSION_ID = "testowa-sesja-zadania-2026-07-15"

    def setUp(self) -> None:
        require_ffmpeg(self)
        self.temporary = tempfile.TemporaryDirectory()
        self.base = Path(self.temporary.name)
        self.live_root = self.base / "live-project"
        (self.live_root / "source-images").mkdir(parents=True)
        (self.live_root / "generation-package" / "upload-derivatives").mkdir(parents=True)
        (self.live_root / "provider").mkdir(parents=True)
    ...
    def _manifest(self, *, cost_status: str = "known", amount: object = 5.0,
        ...
        original = self.live_root / "source-images" / "syntetyczny.png"
        derivative = (
            self.live_root
            / "generation-package"
            / "upload-derivatives"
            / "scn_test00000001-upload.jpg"
        )
        original.write_bytes(b"syntetyczny-original")
        derivative.write_bytes(b"syntetyczny-derivative")
        entry = {
            ...
            "upload_derivative_sha256": sha256_file(derivative),
        ...
        profile_path = self.live_root / "provider" / "provider-profile.snapshot.json"
        atomic_write_json(profile_path, profile)
        profile_sha256 = sha256_file(profile_path)
        session_scope = session_scope_sha256(self.SESSION_ID)
    ...
    def _consent(manifest: dict) -> dict:
        return {
            ...
            "cost_confirmed": True,
            "session_scope_sha256": manifest["session_scope_sha256"],
        }
    ...
    def test_nieznany_koszt_wymaga_dokladnego_komunikatu_i_zgody_na_ryzyko(self) -> None:
        ...
        blocked = evaluate_generation_gate(
            manifest,
            consent,
            project_root=self.live_root,
            current_session_id=self.SESSION_ID,
        ...
        allowed = evaluate_generation_gate(
            manifest,
            consent,
            project_root=self.live_root,
            current_session_id=self.SESSION_ID,
    ...
    def test_przeterminowany_snapshot_blokuje_autoryzacje_w_chwili_uzycia(self) -> None:
        manifest = self._manifest()
        profile_path = self.live_root / manifest["provider_profile_snapshot_path"]
        profile = load_json(profile_path)
        profile["verified_at"] = (
            datetime.now(timezone.utc) - timedelta(days=8)
        ).isoformat().replace("+00:00", "Z")
        atomic_write_json(profile_path, profile)
        profile_sha256 = sha256_file(profile_path)
        manifest["provider_profile_sha256"] = profile_sha256
        manifest["fingerprint_payload"]["provider_profile_sha256"] = profile_sha256
        manifest["batch_fingerprint"] = canonical_hash(manifest["fingerprint_payload"])

        result = evaluate_generation_gate(
            manifest,
            self._consent(manifest),
            project_root=self.live_root,
            current_session_id=self.SESSION_ID,
        )

        self.assertFalse(result["allowed"])
        self.assertTrue(any("nieaktualny" in reason for reason in result["reasons"]))

    def test_zgoda_jest_wazna_tylko_w_biezacej_sesji_zadania(self) -> None:
        manifest = self._manifest()
        consent = self._consent(manifest)
        self.assertTrue(
            evaluate_generation_gate(
        ...
        missing = evaluate_generation_gate(
            manifest,
            consent,
            project_root=self.live_root,
        )
        other = evaluate_generation_gate(
            manifest,
            consent,
            project_root=self.live_root,
            current_session_id="inna-sesja-zadania",
        )
        changed_consent = dict(consent)
        changed_consent["session_scope_sha256"] = session_scope_sha256(
            "inna-sesja-zadania"
        )
        mismatched = evaluate_generation_gate(
            manifest,
            changed_consent,
            project_root=self.live_root,
            current_session_id=self.SESSION_ID,
        ...
        self.assertTrue(any("sesji" in reason for reason in other["reasons"]))
    ...
    def test_live_derivative_musi_istniec_i_zachowac_hash(self) -> None:
        manifest = self._manifest()
        consent = self._consent(manifest)
        derivative = self.live_root / manifest["entries"][0]["upload_derivative_path"]

        derivative.write_bytes(b"zmieniona-zawartosc")
        changed = evaluate_generation_gate(
            manifest,
            consent,
            project_root=self.live_root,
            current_session_id=self.SESSION_ID,
        ...
        self.assertTrue(any("Hash derivative" in reason for reason in changed["reasons"]))
        ...
        self.assertFalse(missing["allowed"])
        self.assertTrue(any("nie istnieje" in reason for reason in missing["reasons"]))

    def test_bramka_bez_katalogu_projektu_jest_fail_closed(self) -> None:
        manifest = self._manifest()
        result = evaluate_generation_gate(
            manifest,
            self._consent(manifest),
        ...
        self.assertFalse(result["allowed"])
        self.assertIn(
            "Brak katalogu projektu do końcowej weryfikacji derivative'ów.",
            result["reasons"],
        )

    @unittest.skipIf(sys.platform.startswith("win"), "Symlinki wymagają uprawnień Windows.")
    def test_symlink_provider_i_derivative_dir_blokuja_publikacje_poza_projektem(self) -> None:
        for child in ("provider", "generation-package/upload-derivatives"):
            with self.subTest(child=child):
                project_root = prepare_synthetic_project(
                    self.base / child.replace("/", "-"), 1
                )
                blocked = project_root / child
                outside = self.base / ("outside-" + child.replace("/", "-"))
                outside.mkdir()
                if blocked.exists():
